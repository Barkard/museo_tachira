...
export default function Edit({ status, pieces, users }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        piece_id: status.piece_id,
        evaluation_date: status.evaluation_date,
        status_details: status.status_details,
        applied_intervention: status.applied_intervention || '',
        user_id: status.user_id,
    });

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('conservacion.update', status.id));
    };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                <div className="max-w-4xl mx-auto w-full bg-white p-6 rounded-lg shadow border border-gray-200">
                    <h2 className="text-xl font-bold mb-6 text-gray-800 border-b pb-2">Editar Reporte de Conservación</h2>
                    ...
                    <form onSubmit={submit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-sm font-medium text-gray-700 mb-1">Intervención Aplicada</label>
                                <textarea
