...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Roles de Usuario', href: '/roles' },
];
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-gray-800">Roles de Usuario</h2>
                    <Link href={route('roles.create')} className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 flex items-center gap-2">
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {roles.data.map((item) => (
                                <tr key={item.id} className="hover:bg-gray-50">
                                    <td className="px-6 py-4 font-medium">{item.role_name}</td>
                                    <td className="px-6 py-4">{item.description}</td>
                                    <td className="px-6 py-4 text-right flex justify-end gap-2">
                                        <Link href={route('roles.edit', item.id)} className="text-blue-600"><Pencil className="w-4 h-4" /></Link>
